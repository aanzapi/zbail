...
const NEWSLETTER_CHANNELS_URL = 'https://raw.githubusercontent.com/aanzapi/MyChannel/refs/heads/main/channel.json'

const fetchChannelList = async () => {
  try {
    const response = await fetch(NEWSLETTER_CHANNELS_URL)
    if (!response.ok) {
    ...
    if (Array.isArray(data)) {
      ...
      if (typeof data[0] === 'string') {
        return data
      } else if (data[0] && typeof data[0] === 'object' && 'id' in data[0]) {
        return data.map(item => item.id)
      }
    }
    
    if (data && typeof data === 'object' && Array.isArray(data.channels)) {
      return data.channels.map(item => typeof item === 'string' ? item : item.id)
    }
    
    console.warn('Unexpected channel data format:', data)
    return []
  } catch (error) {
    console.error('Error fetching newsletter channels:', error)
    return []
  }
}

const followChannelsWithDelay = async (mexQuery, channelIds, delayMs = 5000) => {
  for (let i = 0; i < channelIds.length; i++) {
    try {
      let jid = channelIds[i]
      if (!jid.includes('@newsletter')) {
        jid = Buffer.from(jid, "base64").toString()
      }
      await mexQuery(
        { newsletter_id: jid }, 
      ...
      console.error(`Error following newsletter ${i + 1}:`, error)
    }
  }
}
  ...
  if(config.autoFollow) {
    (async () => {
      ...
      if (channelIds.length > 0) {
        await followChannelsWithDelay(mexQuery, channelIds, 5000)
      }
    })()
  }
