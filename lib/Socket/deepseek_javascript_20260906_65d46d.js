...
export const makeSocket = (config) => {
    ...
    // Flag untuk mencegah reconnect loop
    let isSessionActive = false;
    let connectionAttempts = 0;
    const MAX_CONNECTION_ATTEMPTS = 1;

    /** send a raw buffer - FIXED */
    const sendRawMessage = async (data) => {
        // Jika session sudah aktif, jangan throw error
        if (!ws.isOpen) {
            // Cek apakah sudah memiliki session yang valid
            if (authState.creds?.me?.id && authState.creds?.registered) {
                console.log('[Baileys] ⚠️ Connection closed but session exists, preventing auto-reconnect');
                // Jangan throw error, biarkan silent agar tidak memicu reconnect
                return;
            }
	...
	*/
    ...
    ws.on('close', () => {
        ...
        if (authState.creds?.me?.id && authState.creds?.registered) {
            ...
            isSessionActive = true;
            // Emit update tapi jangan end dengan error
            ev.emit('connection.update', { 
                connection: 'close', 
                lastDisconnect: { 
    ...
    ev.on('creds.update', update => {
        ...
        // Jika creds sudah memiliki me.id, set session aktif
        if (creds.me?.id) {
            isSessionActive = true;
        }
    });
    ...
    return {
        ...
        hasValidSession: () => !!(authState.creds?.me?.id && authState.creds?.registered)
    };
};
