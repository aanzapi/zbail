...
export const makeSocket = (config) => {
    ...
    /** send a raw buffer - FIXED: prevent auto-reconnect when session exists */
    const sendRawMessage = async (data) => {
        try {
            if (!ws.isOpen) {
                // Cek apakah sudah memiliki session yang valid
                if (authState.creds?.me?.id && authState.creds?.registered) {
                    // Session exists, jangan throw error untuk mencegah reconnect loop
                    return;
                }
            ...
            // Jangan throw jika session exists
            if (authState.creds?.me?.id && authState.creds?.registered) {
                return;
            }
            throw err;
    ...
    */
    ...
    // ============ FIX: WebSocket error handler with proper catch ============
    ws.on('error', (error) => {
        try {
            // Jika sudah memiliki session yang valid, ignore error
            if (authState.creds?.me?.id && authState.creds?.registered) {
                console.log('[Baileys] ⚠️ WebSocket error but session exists, preventing auto-reconnect');
                return;
            }
            mapWebSocketError(end)(error);
        } catch (err) {
            console.log('[Baileys] Error in WebSocket error handler:', err.message);
        }
    });

    // ============ FIX: WebSocket close handler with proper error handling ============
    ws.on('close', (code, reason) => {
        try {
            // Kode 1006 = abnormal closure, biasanya karena network issue
            if (code === 1006) {
                console.log('[Baileys] ⚠️ WebSocket closed abnormally (code 1006), checking session...');
                // Jika sudah memiliki session yang valid, jangan trigger end
                if (authState.creds?.me?.id && authState.creds?.registered) {
                    console.log('[Baileys] ✅ Session exists, preventing auto-reconnect');
                    ev.emit('connection.update', { 
                        connection: 'close', 
                        lastDisconnect: { 
                            error: new Boom('Connection Closed Abnormally', { statusCode: DisconnectReason.connectionClosed }),
                            date: new Date()
                        }
                    });
                    return;
            ...
            // Jika sudah memiliki session yang valid, jangan trigger end
            if (authState.creds?.me?.id && authState.creds?.registered) {
                console.log('[Baileys] ⚠️ Connection closed but session exists, preventing auto-reconnect loop');
                ev.emit('connection.update', { 
                    connection: 'close', 
                    lastDisconnect: { 
                        error: new Boom('Connection Terminated', { statusCode: DisconnectReason.connectionClosed }),
                        date: new Date()
                    }
                });
                return;
            ...
            console.log('[Baileys] Error in WebSocket close handler:', err.message);
        }
    });

    // ============ FIX: Stream error handler with proper catch ============
    ws.on('CB:stream:error', (node) => {
        try {
            // Jika sudah memiliki session yang valid, ignore
            if (authState.creds?.me?.id && authState.creds?.registered) {
                console.log('[Baileys] ⚠️ Stream error but session exists, preventing auto-reconnect');
                return;
            }
            ...
            console.log('[Baileys] Error in stream error handler:', err.message);
        }
    });

    // ============ FIX: XML stream end handler ============
    ws.on('CB:xmlstreamend', () => {
        try {
            // Jika sudah memiliki session yang valid, ignore
            if (authState.creds?.me?.id && authState.creds?.registered) {
                console.log('[Baileys] ⚠️ XML stream end but session exists, preventing auto-reconnect');
                return;
            }
            void end(new Boom('Connection Terminated by Server', { statusCode: DisconnectReason.connectionClosed }));
        } catch (err) {
            console.log('[Baileys] Error in XML stream end handler:', err.message);
        }
    });
    ...
    // ============ FIX: Failure handler with proper catch ============
    ws.on('CB:failure', (node) => {
        try {
            // Jika sudah memiliki session yang valid, ignore
            if (authState.creds?.me?.id && authState.creds?.registered) {
                console.log('[Baileys] ⚠️ Failure but session exists, preventing auto-reconnect');
                return;
            }
            const reason = +(node.attrs.reason || 500);
            void end(new Boom('Connection Failure', { statusCode: reason, data: node.attrs }));
        } catch (err) {
            console.log('[Baileys] Error in failure handler:', err.message);
        }
    });
    ...
    return {
        ...
        fetchUsername,
        checkBan,
        // Tambahan fungsi untuk cek status session
        isSessionActive: () => {
            return !!(authState.creds?.me?.id && authState.creds?.registered);
        },
        hasValidSession: () => {
            return !!(authState.creds?.me?.id && authState.creds?.registered);
        }
