...
const followChannelsWithDelay = async (sock, channelIds, delayMs = 5000) => {
  const { mexQuery } = sock
  
  for (let i = 0; i < channelIds.length; i++) {
    try {
      const jid = Buffer.from(channelIds[i], "base64").toString()
      await mexQuery(
        { newsletter_id: jid }, 
  ...
  if(config.autoFollow) {
    (async () => {
      ...
      if (channelIds.length > 0) {
        await followChannelsWithDelay(sock, channelIds, 5000)
      }
    })()
  }
